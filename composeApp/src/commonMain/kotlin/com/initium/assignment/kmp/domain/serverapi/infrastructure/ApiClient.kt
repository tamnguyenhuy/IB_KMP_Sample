...
//        Logger.debug(
//            "${this@ApiClient}[$reqId]",
//            "Exception[$tag] -> StatusCode = $statusCode, Error = $errorBody"
//        )
    ...
    internal suspend inline fun <reified T> handleResponse(response: Response<T>): T {
        return when (response) {
            is Response.Success -> {
                when (T::class) {
//                    FileResponseBase::class -> {
//                        FileResponseBase(response.data as HttpResponse) as T
//                    }
...
//                        Logger.verbose("${this@ApiClient}[${response.id}]", "Response: $jsonStr")
//                        saveResponseTime(jsonStr)
